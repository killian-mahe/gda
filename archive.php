...
<html>
  ...
  <form action="archive.php" method="POST">
    <input type="date" name="semaine">
    <input type="submit" name="archive" value="Choisir" class="button">
  </form>
...
<div id="presence">
  ...
  $good_format = strtotime (strftime($_POST['semaine']));
  $nomfichier = 'Archive/Semaine '.date ('W', $good_format).'.arc';

  $fichier = fopen($nomfichier, 'r');
  $contenu_fichier = file_get_contents($nomfichier);
 
  for ($i=0; $i <= substr_count($contenu_fichier, "\n"); $i++) { 
